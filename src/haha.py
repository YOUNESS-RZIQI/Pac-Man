...
maze = Maze(MAZE_ROWS, MAZE_COLS, 42)
...
OFFSET_X = (WIDTH - MAZE_COLS * CELL) // 2
...
def draw_line(sx: int, sy: int, ex: int, ey: int, color: int) -> None:
    ...
    for i in range(steps + 1):
        t = i / steps
        put_pixel(round(sx + (ex - sx) * t),
                  round(sy + (ey - sy) * t), color)
